...
class CabLists extends Component {
  constructor(props) {
    super(props);
  ...
  getBuserName(value){
    ...
    let url = 'api/cabDisps/?rowCount=10&pageIndex=1&listType=1&userName='+userName+'&wxtoken='+wxtoken;
  ...
  renderLists() {
    ...
    return value.map(datas =>{
        return <li className="cabListLi" key={datas.cabDisp}>
            <dl>
                <dd className="cabListLi-msg weui-flex">
                    <p className="msg-p weui-flex__item">{datas.servName}</p>
                    <p className=" weui-flex__item">{datas.carrName}</p>
                </dd>
                <dd className="cabListLi-msg weui-flex">
                    <p className="msg-p-all weui-flex__item">{datas.depaPortName}------{datas.destPortName}</p>
                </dd>
                {
                   datas.showname == 0? 
                    <dd className="cabListLi-msg  weui-flex ">
                   ...
                   <dd className="cabListLi-msg  weui-flex ">
                        <p className=" weui-flex__item">{datas.resName}</p>
                        <p className=" weui-flex__item">{datas.resCompAlia}</p>
                    </dd>
                }
                <dd className="cabListLi-msg  weui-flex">
                    <p className=" weui-flex__item">{datas.curr == 1?'￥':'$'}{datas.allDepo}</p>            
                </dd>
            </dl>
  ...
  render() {
    ...
    return (
        <div className="cabMain">
            <BackT tit='平台舱位展示' backonClick={this.props.backMain} ></BackT>
